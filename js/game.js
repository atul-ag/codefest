...
(function(){
  ...
  var hasWonRow       = false;
      ...
      currentPlayer   = 1,  // Player O = O, Player X = X
      currentMoves    = 1,
  ...
  $('.box').click(function(e){
    ...
    // Prevent user from clicking a marked box
    if($this.attr("data-select")) { 
      nm=currentPlayer==1?'O':'X';
      $('.notice').html("Player " + nm + "'s turn - <strong>Already marked</strong>"); 
      return; 
    };

    // Set appropriate attributes/values to box clicked
    if(currentPlayer == 1) {
    ...
    $('#log').append('<br>Player'+nm+' | Row:'+(currentRow+1)+', Col:'+(currentColumn+1));
    ...
    if(currentPlayer == 1) {
      $('.notice').text("Player X's turn");
    } else {
      $('.notice').text("Player O's turn");
    }
  ...
  function showMainMessage() {
    $('.notice').addClass('hide'); $('.overlay').fadeIn('1000'); $('.message').addClass('show');
  }
  ...
  function gameLogic() {
    ...
    if(hasWonRow == true || hasWonColumn == true || hasWonDiagonal1 == true || hasWonDiagonal2 == true) {
      ...
      winnerFound = true;
      if( x >= o) {
        $('.overlay .theMessage').text("Well done Player X, You've won the game!");
      } 
      showMainMessage();
